...
${BCFTOOLS} norm -m -any ${sample}.vcf.gz \
    -Oz \
    -o ${sample}.normed.vcf.gz
${BCFTOOLS} index -t -f ${sample}.normed.vcf.gz

echo "Split multiallelic: Finished"

INPUT_VCF=${sample}.normed.vcf.gz
SAMPLE_ID=${sample}.VEP
...
# Generate tsv
echo -e "CHROM\tPOS\tREF\tALT\tDP\t$(${BCFTOOLS} +split-vep -l ${SAMPLE_ID}.vcf | cut -f 2 | tr '\n' '\t' | sed 's/\t$//')" > ${SAMPLE_ID}.protein.tsv
${BCFTOOLS} +split-vep -f '%CHROM\t%POS\t%REF\t%ALT\t%DP\t%CSQ\n' -A tab ${SAMPLE_ID}.vcf >> ${SAMPLE_ID}.protein.tsv

awk 'NR==1 || ($1 ~ /^chr[1-9]$|^chr10$/) {
    gsub(/,.*$/, "", $6)
    print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $9 "\t" $10 "\t" $34 "\t" $44 "\t" $50 "\t" $54 "\t" $55 
}' ${SAMPLE_ID}.protein.tsv > ${SAMPLE_ID}_filtered.protein.tsv
